...
1. Construct a bipartite graph $G = (S \enskip \dot{\bigcup} \enskip R, \enskip E)$ s.t. S and R represent subsets $W$ and $W'$ respectively and an edge is added from $s \in S$ to $r \in R$ if there is a message to be sent from corresponding $W$ to $W'$.
...
2. Construct a bipartite graph $G' = (S \enskip \dot{\bigcup} \enskip R, \enskip E)$ s.t. S and R represent nodes in the current subset and subsets $W'$ respectively. We then use the coloring of the graph constructed in Round 3 as follows. 
...
4. Now construct one more graph $G'' = (S \enskip \dot{\bigcup} \enskip R, \enskip E)$ where $S$ and $R$ represent nodes of the current subset in their sending and receiving role.
